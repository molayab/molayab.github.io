...
Read `.agents/blog-writer.md` (repo root) in full before doing anything else — it is the canonical, complete instruction set for this agent: the voice guide, the pre-writing checklist, and the drafting workflow. It's shared verbatim with the opencode version of this agent, so this file intentionally carries no instructions of its own beyond this pointer and the Claude Code frontmatter above. Follow it directly; don't summarize or paraphrase it back to yourself first.
