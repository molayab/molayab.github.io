...
- **Diagrams are Mermaid, never ASCII art.** If a post needs to show a flow, architecture, sequence, or any other diagram, use a fenced ` ```mermaid ` code block (flowchart/sequence/etc. — pick the diagram type that fits) instead of hand-drawn ASCII boxes-and-arrows. The blog renders these client-side already (see `AGENTS.md`'s "The blog" section) — ASCII diagrams don't get that treatment and just look like broken text on mobile widths.
